
library(here)
source(file = here::here("functions/fun.R"))
f_load_libraries_colors()

palette <- colorRampPalette(c("#5E81AC", "#88C0D0"))
col_mix <- Nord_polar[4]
pal_ratio <- c("#88C0D0", col_mix, col_mix, col_mix, "#5E81AC",col_mix)
Val_Ctrl <- 1e-4
Label_Ctrl <- "0 (Ctrl)"

# 1. Data ----

# Retriving EC50 
df_drc_param <- read.csv(here::here("data/DRC_parameters_cocoons_really_used.csv"))
EPX_EC50_drc <- df_drc_param$coef.drc.4.[3]
IMD_EC50_drc <- df_drc_param$coef.drc.4.[4]

df_mix_ind <- read_excel(here::here("data/Data_Mix.xlsx"), sheet = "ReproSpring2025weight") |>
  mutate(
    Condition = paste0(Ratio, Line),
    Nb_rep = as.factor(Nb_rep),
    w = as.numeric(w),
    L = w^(1 / 3),
    TU_drc = Dose_IMD / IMD_EC50_drc + Dose_EPX / EPX_EC50_drc,
    Dose_IMD_plot = case_when(
      Dose_IMD == 0 ~ Val_Ctrl,
      !(Dose_IMD == 0) ~ Dose_IMD
    ),
    Dose_EPX_plot = case_when(
      Dose_EPX == 0 ~ Val_Ctrl,
      !(Dose_EPX == 0) ~ Dose_EPX
    ),
    TU_drc_plot = case_when(
      TU_drc == 0 ~ Val_Ctrl,
      !(TU_drc == 0) ~ TU_drc
    )
  )

df_mix_coc <- read_excel(here::here("data/Data_Mix.xlsx"), sheet = "ReproSpring2025coc") |>
  mutate(
    Condition = paste0(Ratio, Line),
    w_coc = w_coc_tot / (Nb_cocoons - Nb_cocoons_crushed),
    TU_drc = Dose_IMD / IMD_EC50_drc + Dose_EPX / EPX_EC50_drc,
    Dose_IMD_plot = case_when(
      Dose_IMD == 0 ~ Val_Ctrl,
      !(Dose_IMD == 0) ~ Dose_IMD
    ),
    Dose_EPX_plot = case_when(
      Dose_EPX == 0 ~ Val_Ctrl,
      !(Dose_EPX == 0) ~ Dose_EPX
    ),
    TU_drc_plot = case_when(
      TU_drc == 0 ~ Val_Ctrl,
      !(TU_drc == 0) ~ TU_drc
    )
  )

df_coc_mean_controls <- subset(df_mix_coc, Dose_IMD == 0 & Dose_EPX == 0) |> 
  aggregate(Nb_cocoons ~ Lot, FUN = mean)

Mean_coc_controls <- mean(subset(df_mix_coc, Dose_IMD == 0 & Dose_EPX == 0)$Nb_cocoons)

#Shift <- 1e-6

df_mix_coc <- df_mix_coc |> 
  mutate(
    r_Nb_cocoons = Nb_cocoons/Mean_coc_controls
  )

df_mix_coc <- df_mix_coc |>
  mutate(
    Trans_r_Nb_cocoons = sqrt(r_Nb_cocoons)
  )

df_conc_mix <- df_mix_coc |> 
  dplyr::select(Condition, Dose_IMD, Dose_EPX)%>%
  distinct(Condition, .keep_all = TRUE)

df_mix_cocsize <- read_excel(here::here("data/Data_Mix.xlsx"), sheet="ReproSpring2025cocsize") |> 
  mutate(
    Condition = paste0(Ratio, Line),
    v_coc = 4/3*pi*(Coc_height/2)*(Coc_width/2)^2,
  ) |> 
  left_join(df_conc_mix, by = "Condition") |> 
  mutate(TU_drc = Dose_IMD / IMD_EC50_drc + Dose_EPX/EPX_EC50_drc)

df_mix_cocsize_cosm <- df_mix_cocsize |> 
  aggregate(v_coc ~ Ratio + Line + Nb_rep + TU_drc, FUN = mean) |> 
  mutate(Condition = paste0(Ratio, Line))

df_mix_coc <- df_mix_coc |> 
  full_join(df_mix_cocsize_cosm, by = c("Condition", "Nb_rep", "Ratio", "Line", "TU_drc")) |> 
  mutate(
    Id_Cond = w_coc/v_coc
  )

df_mix_coc_mean <- df_mix_coc |>
  dplyr::group_by(Condition, Dose_IMD, Dose_EPX, Dose_IMD_plot, Dose_EPX_plot, Ratio, Line, TU_drc) |>
  dplyr::summarise(
    Nb_cocoons = mean(Nb_cocoons, na.rm = TRUE),
    w_coc = mean(w_coc, na.rm = TRUE),
    v_coc = mean(v_coc, na.rm = TRUE),
    r_Nb_cocoons = mean(r_Nb_cocoons, na.rm = TRUE),
    Trans_r_Nb_cocoons = mean(Trans_r_Nb_cocoons, na.rm = TRUE),
    #Nb_cocoons_normalized = mean(Nb_cocoons_normalized, na.rm = TRUE),
    .groups = "drop"
  )

df_mix_coc_IMD_mean <- subset(df_mix_coc_mean, Dose_EPX ==0)
df_mix_coc_EPX_mean <- subset(df_mix_coc_mean, Dose_IMD ==0)


df_mix_coc_sd <- df_mix_coc |>
  dplyr::group_by(Condition, Dose_IMD, Dose_EPX, Dose_IMD_plot, Dose_EPX_plot, Ratio, Line, TU_drc) |>
  dplyr::summarise(
    Nb_cocoons = sd(Nb_cocoons, na.rm = TRUE),
    w_coc = sd(w_coc, na.rm = TRUE),
    v_coc = sd(v_coc, na.rm = TRUE),
    r_Nb_cocoons = sd(r_Nb_cocoons, na.rm = TRUE),
    Trans_r_Nb_cocoons = sd(Trans_r_Nb_cocoons, na.rm = TRUE),
    #Nb_cocoons_normalized = mean(Nb_cocoons_normalized, na.rm = TRUE),
    .groups = "drop"
  )

df_mix_coc_cv <- df_mix_coc_mean |>
  dplyr::rename_with(~ paste0(.x, "_mean"), -c(Condition, Dose_IMD, Dose_EPX, Dose_IMD_plot, Dose_EPX_plot, Ratio, Line, TU_drc)) |>
  dplyr::left_join(
    df_mix_coc_sd |> 
      dplyr::rename_with(~ paste0(.x, "_sd"), -c(Condition, Dose_IMD, Dose_EPX, Dose_IMD_plot, Dose_EPX_plot, Ratio, Line, TU_drc)),
    by = c("Condition", "Dose_IMD", "Dose_EPX", "Dose_IMD_plot", "Dose_EPX_plot", "Ratio", "Line", "TU_drc")
  ) |>
  dplyr::mutate(
    CV_Nb_cocoons = Nb_cocoons_sd / Nb_cocoons_mean,
    CV_w_coc = w_coc_sd / w_coc_mean,
    CV_v_coc = v_coc_sd / v_coc_mean,
    CV_r_Nb_cocoons = r_Nb_cocoons_sd / r_Nb_cocoons_mean,
    CV_Trans_r_Nb_cocoons = Trans_r_Nb_cocoons_sd / Trans_r_Nb_cocoons_mean
  )

## Dispersion ----

df_mix_coc <- df_mix_coc |> 
  mutate(Condition_f = as.factor(Condition))

m.Poisson <- glm(Nb_cocoons ~ Condition_f, family = poisson, data = df_mix_coc)
dispersion <- sum(residuals(m.Poisson, type = "pearson")^2) / m.Poisson$df.residual
dispersion
library(AER)
dispersiontest(m.Poisson)


#m.QPoisson <- survey::svyglm(Nb_cocoons ~ Condition_f, family = quasipoisson, data = df_mix_coc)
m.NB <- glm.nb(Nb_cocoons ~ Condition_f, data = df_mix_coc)

# 1. Calculer moyenne et variance par groupe
library(dplyr)

variance_mean <- df_mix_coc %>%
  group_by(Condition_f) %>%
  summarise(
    mean = mean(Nb_cocoons),
    variance = var(Nb_cocoons),
    n = n()
  )

theta_nb <- summary(m.NB)$theta

# 3. Créer les données pour les courbes théoriques
mean_range <- seq(0, max(variance_mean$mean) * 1.1, length.out = 100)

theoretical <- data.frame(
  mean = rep(mean_range, 3),
  variance = c(
    mean_range,  # Poisson
    dispersion * mean_range,  # Quasi-Poisson
    mean_range + mean_range^2 / theta_nb  # Negative Binomial
  ),
  model = rep(c("Poisson", "Quasi-Poisson", "Binomiale Negative"), 
              each = length(mean_range))
)

# 4. Créer le graphique
ggplot() +
  # Lignes théoriques
  geom_line(data = theoretical, 
            aes(x = mean, y = variance, color = model, linetype = model), 
            linewidth = 1.2) +
  # Points observés
  geom_point(data = variance_mean, 
             aes(x = mean, y = variance), 
             size = 4, color = "black", alpha = 0.5) +
  # Labels des conditions
  geom_text(data = variance_mean, 
            aes(x = mean, y = variance, label = Condition_f), 
            vjust = -1.2, hjust = 0.5, size = 3.5, fontface = "bold") +
  # Mise en forme
  labs(x = "Moyenne (μ)", 
       y = "Variance (σ²)", 
       title = "Relation Variance-Moyenne : Choix du modèle de distribution",
       subtitle = "Les points noirs représentent les valeurs observées par condition",
       color = "Distribution théorique",
       linetype = "Distribution théorique") +
  theme_minimal(base_size = 12) +
  theme(
    legend.position = "bottom",
    legend.title = element_text(face = "bold"),
    plot.title = element_text(face = "bold", size = 14),
    plot.subtitle = element_text(size = 10, color = "gray30")
  ) +
  scale_color_manual(values = c(
    "Poisson" = "gray50", 
    "Quasi-Poisson" = "#E41A1C", 
    "Binomiale Negative" = "#377EB8"
  )) +
  scale_linetype_manual(values = c(
    "Poisson" = "dashed",
    "Quasi-Poisson" = "solid",
    "Binomiale Negative" = "solid"
  )) +
  guides(color = guide_legend(nrow = 3), 
         linetype = guide_legend(nrow = 3))

# Afficher aussi le tableau des valeurs
print(variance_mean)

## Preparation of model entry ----

df_design <- read.csv(here::here("data/Design_mixture.csv"))




# slope <- coef(drc.mix.IE)[1]
# Y_max <- coef(drc.mix.IE)[2]
# EPX_EC50 <- coef(drc.mix.IE)[3]
# IMD_EC50 <- coef(drc.mix.IE)[4]
# Y_min <- 0

df_mix_coc_Cleo <- df_mix_coc |> 
  mutate(Response = Nb_cocoons) |> 
  filter(Ratio %in% c("I", "E", "N")) |> 
  dplyr::select(Dose_EPX, Dose_IMD, Response, Line, Ratio, Nb_rep) |> 
  as.data.frame() 

write_csv(df_mix_coc_Cleo, here::here("Data_MIX_Cleo.csv"))

df_mix_coc_int <- df_mix_coc |> 
  mutate(Response = Nb_cocoons) |> 
  dplyr::select(Dose_EPX, Dose_IMD, Response, Line, Ratio, TU_drc, Nb_rep) |> 
  as.data.frame() |> 
  filter(!(Dose_IMD == 0 & Dose_EPX == 0))

df_mix_coc_int_single <- df_mix_coc_int |> 
  filter(Dose_EPX == 0 | Dose_IMD == 0)

drc_fit_NB<-CA_complete_fit_speed(                                    
  C_mat=cbind(                                                   
    df_mix_coc_int_single$Dose_EPX,                                  
    df_mix_coc_int_single$Dose_IMD                                  
  ),                                                   
  Response=df_mix_coc_int_single$Response,                          
  interact="none",
  identical_slopes = TRUE,
  start = c(14, 2, 200, 0.5),
  upper = c(30, 10, 400, 50),
  error_type = "NB",
  multicore = TRUE,                                        
  mc.cores = 4                                             
) |>                                                      
  as.data.frame()

drc_fit_Poisson<-CA_complete_fit_speed(                                    
  C_mat=cbind(                                                   
    df_mix_coc_int_single$Dose_EPX,                                  
    df_mix_coc_int_single$Dose_IMD                                  
  ),                                                   
  Response=df_mix_coc_int_single$Response,                          
  interact="none",
  identical_slopes = TRUE,
  start = c(14, 2, 200, 0.5),
  upper = c(30, 10, 400, 50),
  error_type = "Poisson",
  multicore = TRUE,                                        
  mc.cores = 4                                             
) |>                                                      
  as.data.frame()


df_mix_coc_IMD <- subset(df_mix_coc, Ratio %in% c("I", "N")) |> 
  mutate(
    Dose = Dose_IMD,
    Dose_plot = Dose_IMD_plot,
    Molecule = "IMD"
  ) |> 
  dplyr::select(Dose, Dose_plot, Nb_cocoons, Molecule, Line)

# We do not take the controls into account twice
df_mix_coc_EPX <- subset(df_mix_coc, Ratio  %in% c("E")) |> 
  mutate(
    Dose = Dose_EPX,
    Dose_plot = Dose_EPX_plot,
    Molecule = "EPX"
  ) |>  
  dplyr::select(Dose, Dose_plot, Nb_cocoons, Molecule, Line)

df_mix_coc_EPX_plot <- subset(df_mix_coc, Ratio  %in% c("E", "N")) |>
  mutate(
    Dose = Dose_EPX,
    Dose_plot = Dose_EPX_plot,
    Molecule = "EPX"
  ) |>
  dplyr::select(Dose, Dose_plot, Nb_cocoons, Molecule)

df_mix_coc_single <- rbind(df_mix_coc_IMD, df_mix_coc_EPX)

df_mix_coc_single_wocontrols <- df_mix_coc_single |> 
  filter(!Dose == 0)

drc.mix.IE<- drm(
  Nb_cocoons~Dose, Molecule, 
  data = df_mix_coc_single_wocontrols,
  type="Poisson",
  fct=LL.4(
    names = c("slope", "Ymin", "Ymax", "EC50"),
    fixed = c(NA, 0, NA, NA)
  ), 
  # Reproduction is expected to reach 0 for large concentrations
  pmodels=data.frame(1, 1, Molecule)
)

Hill.mix.IE <- function(Dose, Molecule){
  
  i <- ifelse(Molecule == "IMD", 1, 0)
  
  Y_min <- 0
  slope <- coef(drc.mix.IE)[1]
  Y_max <- coef(drc.mix.IE)[2]
  EC50 <- coef(drc.mix.IE)[3+i]
  
  return(Y_min+(Y_max-Y_min)/(1+exp(slope*(log(Dose)-log(EC50)))))
}

df_mix_coc_single_wocontrols <- df_mix_coc_single_wocontrols |> 
  mutate(
    Nb_cocoons_pred = Hill.mix.IE(Dose, Molecule)
  )

LL_drc_Poisson = sum(dpois(df_mix_coc_single_wocontrols$Nb_cocoons, lambda = df_mix_coc_single_wocontrols$Nb_cocoons_pred, log = TRUE))

C_mat = cbind(df_design$EPX,df_design$IMD)
Max_NB = drc_fit_NB$Max[1]
Slopes_NB = drc_fit_NB$Slopes
Ec50s_NB = drc_fit_NB$Ec50s

param_NB <- data.frame(Slopes=Slopes_NB, Max=Max_NB, Ec50s=Ec50s_NB)

Max_Poisson = drc_fit_Poisson$Max[1]
Slopes_Poisson = drc_fit_Poisson$Slopes
Ec50s_Poisson = drc_fit_Poisson$Ec50s

param_Poisson <- data.frame(Slopes=Slopes_Poisson, Max=Max_Poisson, Ec50s=Ec50s_Poisson)

min_dose_EPX <- min(subset(df_design, !EPX==0)$EPX)
max_dose_EPX <- max(subset(df_design, !EPX==0)$EPX)

min_dose_IMD <- min(subset(df_design, !IMD==0)$IMD)
max_dose_IMD <- max(subset(df_design, !IMD==0)$IMD)

by_EPX <- (log(max_dose_EPX)-log(min_dose_EPX))/100
by_IMD <- (log(max_dose_IMD)-log(min_dose_IMD))/100

# Créer une grille pour x et y en utilisant expand.grid
grid_C1 <- exp(seq(log(min_dose_EPX), log(max_dose_EPX), by = by_EPX))
grid_C2 <- exp(seq(log(min_dose_IMD), log(max_dose_IMD), by = by_IMD))
grid <- expand.grid(
  x = grid_C1, 
  y = grid_C2
)



# 2. Fit des modèles ----

signif_LL <- 5

########### CA #######################################

CA_fit_NB <- data.frame(Error = NA)
CA_fit_NB$Error <- CA_complete2_NB(
  C_mat=cbind(                                                    # <1>
    df_mix_coc_int$Dose_EPX,                                      # <1>
    df_mix_coc_int$Dose_IMD                                       # <1>
  ),                                                            # <1>
  Response=df_mix_coc_int$Response,                               # <1>
  Max = Max_NB,
  Slopes = Slopes_NB,
  Ec50s = Ec50s_NB,
  interact="none",                                                # <1>
  multicore = TRUE, 
  mc.cores = 4
)

CA_fit_NB$Type <- "CA"
CA_fit_NB$Ref <- "CA"

LogLikelihood_CA_NB <- -CA_fit_NB$Error                              
CA_fit_NB$a <- NA
CA_fit_NB$b <- NA
# Printing results
LogLikelihood_CA_NB_print <- signif(LogLikelihood_CA_NB, signif_LL)


CA_fit_Poisson <- data.frame(Error = NA)
CA_fit_Poisson$Error <- CA_complete2_Poisson(
  C_mat=cbind(                                                    # <1>
    df_mix_coc_int$Dose_EPX,                                      # <1>
    df_mix_coc_int$Dose_IMD                                       # <1>
  ),                                                            # <1>
  Response=df_mix_coc_int$Response,                               # <1>
  Max = Max_Poisson,
  Slopes = Slopes_Poisson,
  Ec50s = Ec50s_Poisson,
  interact="none",                                                # <1>
  multicore = TRUE, 
  mc.cores = 4
)

CA_fit_Poisson$Type <- "CA"
CA_fit_Poisson$Ref <- "CA"

LogLikelihood_CA_Poisson <- -CA_fit_Poisson$Error                              
CA_fit_Poisson$a <- NA
CA_fit_Poisson$b <- NA
# Printing results
LogLikelihood_CA_Poisson_print <- signif(LogLikelihood_CA_Poisson, signif_LL)

################# IA ####################################

IA_fit_NB <- data.frame(Error = NA)
IA_fit_NB$Error <- IA_complete2_NB(
  C_mat=cbind(                                                    # <1>
    df_mix_coc_int$Dose_EPX,                                      # <1>
    df_mix_coc_int$Dose_IMD                                       # <1>
  ),                                                            # <1>
  Response=df_mix_coc_int$Response,                               # <1>
  Max = Max_NB,
  Slopes = Slopes_NB,
  Ec50s = Ec50s_NB,
  interact="none"
)

IA_fit_NB$Type <- "IA"
IA_fit_NB$Ref <- "IA"

LogLikelihood_IA_NB <- -IA_fit_NB$Error                              
IA_fit_NB$a <- NA
IA_fit_NB$b <- NA
# Printing results
LogLikelihood_IA_NB_print <- signif(LogLikelihood_IA_NB, signif_LL)


IA_fit_Poisson <- data.frame(Error = NA)
IA_fit_Poisson$Error <- IA_complete2_Poisson(
  C_mat=cbind(                                                    # <1>
    df_mix_coc_int$Dose_EPX,                                      # <1>
    df_mix_coc_int$Dose_IMD                                       # <1>
  ),                                                            # <1>
  Response=df_mix_coc_int$Response,                               # <1>
  Max = Max_Poisson,
  Slopes = Slopes_Poisson,
  Ec50s = Ec50s_Poisson,
  interact="none"
)

IA_fit_Poisson$Type <- "IA"
IA_fit_Poisson$Ref <- "IA"

LogLikelihood_IA_Poisson <- -IA_fit_Poisson$Error                              
IA_fit_Poisson$a <- NA
IA_fit_Poisson$b <- NA
# Printing results
LogLikelihood_IA_Poisson_print <- signif(LogLikelihood_IA_Poisson, signif_LL)


######## CASA ##################################################################

CASA_fit_NB<-CA_complete_fit_speed(                                    # <1>
  C_mat=cbind(                                                    # <1>
    df_mix_coc_int$Dose_EPX,                                      # <1>
    df_mix_coc_int$Dose_IMD                                       # <1>
  ),                                                            # <1>
  Response=df_mix_coc_int$Response,                               # <1>
  interact="SA",                                                  # <1>
  param=param_NB,                                               # <1>
  error_type = "NB",
  multicore = TRUE,                                               # <1>
  mc.cores = 4                                                    # <1>
) |>                                                                # <1>
  as.data.frame()
CASA_fit_NB$Type <- "SA"
CASA_fit_NB$Ref <- "CA"

LogLikelihood_CASA_NB <- -CASA_fit_NB$Error                              
a_CASA_NB <- CASA_fit_NB$a
CASA_fit_NB$b <- NA
# Printing results
a_CASA_NB_print <- signif(a_CASA_NB, 3)
LogLikelihood_CASA_NB_print <- signif(LogLikelihood_CASA_NB, signif_LL)

CASA_fit_Poisson<-CA_complete_fit_speed(                                    # <1>
  C_mat=cbind(                                                    # <1>
    df_mix_coc_int$Dose_EPX,                                      # <1>
    df_mix_coc_int$Dose_IMD                                       # <1>
  ),                                                            # <1>
  Response=df_mix_coc_int$Response,                               # <1>
  interact="SA",                                                  # <1>
  param=param_Poisson,                                                   # <1>
  error_type = "Poisson",
  multicore = TRUE,                                               # <1>
  mc.cores = 4                                                    # <1>
) |>                                                                # <1>
  as.data.frame()
CASA_fit_Poisson$Type <- "SA"
CASA_fit_Poisson$Ref <- "CA"

LogLikelihood_CASA_Poisson <- -CASA_fit_Poisson$Error                              
a_CASA_Poisson <- CASA_fit_Poisson$a
CASA_fit_Poisson$b <- NA
# Printing results
a_CASA_Poisson_print <- signif(a_CASA_Poisson, 3)
LogLikelihood_CASA_Poisson_print <- signif(LogLikelihood_CASA_Poisson, signif_LL)

######## IASA ##################################################################

IASA_fit_NB<-IA_complete_fit_speed(                                    # <1>
  C_mat=cbind(                                                    # <1>
    df_mix_coc_int$Dose_EPX,                                      # <1>
    df_mix_coc_int$Dose_IMD                                       # <1>
  ),                                                            # <1>
  Response=df_mix_coc_int$Response,                               # <1>
  interact="SA",                                                  # <1>
  param=param_NB,                                                 # <1>
  error_type = "NB",
  multicore = TRUE,                                               # <1>
  mc.cores = 4                                                    # <1>
) |>                                                                # <1>
  as.data.frame()
IASA_fit_NB$Type <- "SA"
IASA_fit_NB$Ref <- "IA"

LogLikelihood_IASA_NB <- -IASA_fit_NB$Error                              
a_IASA_NB <- IASA_fit_NB$a
IASA_fit_NB$b <- NA
# Printing results
a_IASA_NB_print <- signif(a_IASA_NB, 3)
LogLikelihood_IASA_NB_print <- signif(LogLikelihood_IASA_NB, signif_LL)

IASA_fit_Poisson<-IA_complete_fit_speed(                                    # <1>
  C_mat=cbind(                                                    # <1>
    df_mix_coc_int$Dose_EPX,                                      # <1>
    df_mix_coc_int$Dose_IMD                                       # <1>
  ),                                                            # <1>
  Response=df_mix_coc_int$Response,                               # <1>
  interact="SA",                                                  # <1>
  param=param_Poisson,                                                   # <1>
  error_type = "Poisson",
  multicore = TRUE,                                               # <1>
  mc.cores = 4                                                    # <1>
) |>                                                                # <1>
  as.data.frame()
IASA_fit_Poisson$Type <- "SA"
IASA_fit_Poisson$Ref <- "IA"

LogLikelihood_IASA_Poisson <- -IASA_fit_Poisson$Error                              
a_IASA_Poisson <- IASA_fit_Poisson$a
IASA_fit_Poisson$b <- NA
# Printing results
a_IASA_Poisson_print <- signif(a_IASA_Poisson, 3)
LogLikelihood_IASA_Poisson_print <- signif(LogLikelihood_IASA_Poisson, signif_LL)

# 3. Tests of models ----

df_mix_coc_int_anova <- df_mix_coc_int |> 
  mutate(
    Dose_EPX_f = as.factor(Dose_EPX),
    Dose_IMD_f = as.factor(Dose_IMD),
    Dose_IMD_EPX_f = paste0(Dose_EPX_f, Dose_IMD_f)
  )

anova.mix <- glm(
  Response ~ Dose_IMD_EPX_f, 
  family = poisson(link = "log"), 
  data = df_mix_coc_int_anova
)

anova.mix_NB <- glm.nb(
  Response ~ Dose_IMD_EPX_f, 
  data = df_mix_coc_int_anova
)

# Log-vraisemblances
LL_drc <- LogLikelihood_CA_NB[1]
LL_anova <- as.numeric(logLik(anova.mix_NB))

# Nombre de paramètres
k_drc <- 6
k_anova <- attr(logLik(anova.mix_NB), "df")

# Test du rapport de vraisemblance
# H0 : le modèle drc est adéquat
# H1 : le modèle ANOVA (saturé) s'ajuste mieux
D <- -2 * (LL_drc - LL_anova)
df_test <- k_anova - k_drc

# Vérification
cat("LL_anova:", LL_anova, " - LL_drc:", LL_drc, "\n")
cat("k_anova:", k_anova, "- k_drc:", k_drc, "- df:", df_test, "\n")

p_val <- pchisq(D, df = df_test, lower.tail = FALSE)
cat("Lack-of-fit test CA/Anova: D =", round(D, 3), ", df =", df_test, ", p =", p_val, "\n")


# Log-vraisemblances
LL_drc <- LogLikelihood_IA_NB[1]
LL_anova <- as.numeric(logLik(anova.mix_NB))

# Nombre de paramètres
k_drc <- 5
k_anova <- attr(logLik(anova.mix_NB), "df")

# Test du rapport de vraisemblance
# H0 : le modèle drc est adéquat
# H1 : le modèle ANOVA (saturé) s'ajuste mieux
D <- -2 * (LL_drc - LL_anova)
df_test <- k_anova - k_drc

# Vérification
cat("LL_anova:", LL_anova, " - LL_drc:", LL_drc, "\n")
cat("k_anova:", k_anova, "- k_drc:", k_drc, "- df:", df_test, "\n")

p_val <- pchisq(D, df = df_test, lower.tail = FALSE)
cat("Lack-of-fit test IA/Anova: D =", round(D, 3), ", df =", df_test, ", p =", p_val, "\n")


# Log-vraisemblances
LL_drc <- LogLikelihood_CASA_NB[1]
LL_anova <- as.numeric(logLik(anova.mix_NB))

# Nombre de paramètres
k_drc <- 6
k_anova <- attr(logLik(anova.mix_NB), "df")

# Test du rapport de vraisemblance
# H0 : le modèle drc est adéquat
# H1 : le modèle ANOVA (saturé) s'ajuste mieux
D <- -2 * (LL_drc - LL_anova)
df_test <- k_anova - k_drc

# Vérification
cat("LL_anova:", LL_anova, " - LL_drc:", LL_drc, "\n")
cat("k_anova:", k_anova, "- k_drc:", k_drc, "- df:", df_test, "\n")

p_val <- pchisq(D, df = df_test, lower.tail = FALSE)
cat("Lack-of-fit test CASA/Anova: D =", round(D, 3), ", df =", df_test, ", p =", p_val, "\n")



LL_drc <- LogLikelihood_IASA_NB[1]
LL_anova <- as.numeric(logLik(anova.mix_NB))

# Nombre de paramètres
k_drc <- 5
k_anova <- attr(logLik(anova.mix_NB), "df")

# Test du rapport de vraisemblance
# H0 : le modèle drc est adéquat
# H1 : le modèle ANOVA (saturé) s'ajuste mieux
D <- -2 * (LL_drc - LL_anova)
df_test <- k_anova - k_drc

# Vérification
cat("LL_anova:", LL_anova, " - LL_drc:", LL_drc, "\n")
cat("k_anova:", k_anova, "- k_drc:", k_drc, "- df:", df_test, "\n")

p_val <- pchisq(D, df = df_test, lower.tail = FALSE)
cat("Lack-of-fit test IASA/Anova: D =", round(D, 3), ", df =", df_test, ", p =", p_val, "\n")






# Log-vraisemblances
LL_drc <- LogLikelihood_CA_Poisson[1]
LL_anova <- as.numeric(logLik(anova.mix))

# Nombre de paramètres
k_drc <- 4
k_anova <- attr(logLik(anova.mix), "df")

# Test du rapport de vraisemblance
# H0 : le modèle drc est adéquat
# H1 : le modèle ANOVA (saturé) s'ajuste mieux
D <- -2 * (LL_drc - LL_anova)
df_test <- k_anova - k_drc

# Vérification
cat("LL_anova:", LL_anova, " - LL_drc:", LL_drc, "\n")
cat("k_anova:", k_anova, "- k_drc:", k_drc, "- df:", df_test, "\n")

p_val <- pchisq(D, df = df_test, lower.tail = FALSE)
cat("Lack-of-fit test CA/Anova: D =", round(D, 3), ", df =", df_test, ", p =", p_val, "\n")


# Log-vraisemblances
LL_drc <- LogLikelihood_IA_Poisson[1]
LL_anova <- as.numeric(logLik(anova.mix))

# Nombre de paramètres
k_drc <- 4
k_anova <- attr(logLik(anova.mix), "df")

# Test du rapport de vraisemblance
# H0 : le modèle drc est adéquat
# H1 : le modèle ANOVA (saturé) s'ajuste mieux
D <- -2 * (LL_drc - LL_anova)
df_test <- k_anova - k_drc

# Vérification
cat("LL_anova:", LL_anova, " - LL_drc:", LL_drc, "\n")
cat("k_anova:", k_anova, "- k_drc:", k_drc, "- df:", df_test, "\n")

p_val <- pchisq(D, df = df_test, lower.tail = FALSE)
cat("Lack-of-fit test IA/Anova: D =", round(D, 3), ", df =", df_test, ", p =", p_val, "\n")


# Log-vraisemblances
LL_drc <- LogLikelihood_CASA_Poisson[1]
LL_anova <- as.numeric(logLik(anova.mix))

# Nombre de paramètres
k_drc <- 5
k_anova <- attr(logLik(anova.mix), "df")

# Test du rapport de vraisemblance
# H0 : le modèle drc est adéquat
# H1 : le modèle ANOVA (saturé) s'ajuste mieux
D <- -2 * (LL_drc - LL_anova)
df_test <- k_anova - k_drc

# Vérification
cat("LL_anova:", LL_anova, " - LL_drc:", LL_drc, "\n")
cat("k_anova:", k_anova, "- k_drc:", k_drc, "- df:", df_test, "\n")

p_val <- pchisq(D, df = df_test, lower.tail = FALSE)
cat("Lack-of-fit test CASA/Anova: D =", round(D, 3), ", df =", df_test, ", p =", p_val, "\n")



LL_drc <- LogLikelihood_IASA_Poisson[1]
LL_anova <- as.numeric(logLik(anova.mix))

# Nombre de paramètres
k_drc <- 5
k_anova <- attr(logLik(anova.mix), "df")

# Test du rapport de vraisemblance
# H0 : le modèle drc est adéquat
# H1 : le modèle ANOVA (saturé) s'ajuste mieux
D <- -2 * (LL_drc - LL_anova)
df_test <- k_anova - k_drc

# Vérification
cat("LL_anova:", LL_anova, " - LL_drc:", LL_drc, "\n")
cat("k_anova:", k_anova, "- k_drc:", k_drc, "- df:", df_test, "\n")

p_val <- pchisq(D, df = df_test, lower.tail = FALSE)
cat("Lack-of-fit test IASA/Anova: D =", round(D, 3), ", df =", df_test, ", p =", p_val, "\n")






LL0 <- LogLikelihood_IA_NB    # log-vraisemblance du modèle IA
LL1 <- LogLikelihood_IASA_NB    # log-vraisemblance du modèle (IA reference)

p0 <- 0         # nombre de paramètres additionnels dans le modèle IA (par rapport à ta base)
p1 <- 1         # un paramètre d'interaction dans le modèle SA (IA reference)

D <- -2 * (LL0 - LL1)
p_val <- pchisq(D, df = p1 - p0, lower.tail = FALSE)

cat("Comparison IA/SA - D =", D, "p =", p_val, "\n")

LL0 <- LogLikelihood_CA_NB    # log-vraisemblance du modèle IA
LL1 <- LogLikelihood_CASA_NB    # log-vraisemblance du modèle (IA reference)

p0 <- 0         # nombre de paramètres additionnels dans le modèle IA (par rapport à ta base)
p1 <- 1         # un paramètre d'interaction dans le modèle SA (IA reference)

D <- -2 * (LL0 - LL1)
p_val <- pchisq(D, df = p1 - p0, lower.tail = FALSE)

cat("Comparison CA/SA - D =", D, "p =", p_val, "\n")




LL0 <- LogLikelihood_IA_Poisson    # log-vraisemblance du modèle IA
LL1 <- LogLikelihood_IASA_Poisson    # log-vraisemblance du modèle (IA reference)

p0 <- 0         # nombre de paramètres additionnels dans le modèle IA (par rapport à ta base)
p1 <- 1         # un paramètre d'interaction dans le modèle SA (IA reference)

D <- -2 * (LL0 - LL1)
p_val <- pchisq(D, df = p1 - p0, lower.tail = FALSE)

cat("Comparison IA/SA - D =", D, "p =", p_val, "\n")

LL0 <- LogLikelihood_CA_Poisson    # log-vraisemblance du modèle IA
LL1 <- LogLikelihood_CASA_Poisson    # log-vraisemblance du modèle (IA reference)

p0 <- 0         # nombre de paramètres additionnels dans le modèle IA (par rapport à ta base)
p1 <- 1         # un paramètre d'interaction dans le modèle SA (IA reference)

D <- -2 * (LL0 - LL1)
p_val <- pchisq(D, df = p1 - p0, lower.tail = FALSE)

cat("Comparison CA/SA - D =", D, "p =", p_val, "\n")

# 4. PredObs ----

C_mat=cbind(                                            
  df_mix_coc_int$Dose_EPX,                             
  df_mix_coc_int$Dose_IMD                             
)

# Calculation of effects for the design concentrations 
res_CA_NB <- CA_complete2(
  C_mat, 
  Max_NB, Slopes_NB, Ec50s_NB, 
  interact="none"
)

res_CA_Poisson <- CA_complete2(
  C_mat, 
  Max_Poisson, Slopes_Poisson, Ec50s_Poisson, 
  interact="none"
)

res_CASA_NB <- CA_complete2(
  C_mat, 
  Max_NB, Slopes_NB, Ec50s_NB, 
  interact="SA",
  a = CASA_fit_NB$a
)

res_CASA_Poisson <- CA_complete2(
  C_mat, 
  Max_Poisson, Slopes_Poisson, Ec50s_Poisson, 
  interact="SA",
  a = CASA_fit_Poisson$a
)

res_IA_NB <- IA_complete2(
  C_mat, 
  Max_NB, Slopes_NB, Ec50s_NB, 
  interact="none"
)

res_IA_Poisson <- IA_complete2(
  C_mat, 
  Max_Poisson, Slopes_Poisson, Ec50s_Poisson, 
  interact="none"
)

res_IASA_NB <- IA_complete2(
  C_mat, 
  Max_NB, Slopes_NB, Ec50s_NB, 
  interact="SA",
  a = IASA_fit_NB$a
)

res_IASA_Poisson <- IA_complete2(
  C_mat, 
  Max_Poisson, Slopes_Poisson, Ec50s_Poisson, 
  interact="SA",
  a = IASA_fit_Poisson$a
)

# We add the corresponding response to our design
df_predobs_CA_NB <- df_mix_coc_int%>%
  mutate(Predicted=res_CA_NB)
df_predobs_CASA_NB <- df_mix_coc_int%>%
  mutate(Predicted=res_CASA_NB)

df_predobs_IA_NB <- df_mix_coc_int%>%
  mutate(Predicted=res_IA_NB)
df_predobs_IASA_NB <- df_mix_coc_int%>%
  mutate(Predicted=res_IASA_NB)

df_predobs_CA_Poisson <- df_mix_coc_int%>%
  mutate(Predicted=res_CA_Poisson)
df_predobs_CASA_Poisson <- df_mix_coc_int%>%
  mutate(Predicted=res_CASA_Poisson)

df_predobs_IA_Poisson <- df_mix_coc_int%>%
  mutate(Predicted=res_IA_Poisson)
df_predobs_IASA_Poisson <- df_mix_coc_int%>%
  mutate(Predicted=res_IASA_Poisson)


f_calc_residuals <- function(df){
  df <- df |> 
    mutate(
      Ratio_predobs = Predicted / Response,
      Residuals = Predicted - Response
    )
}

dfs <- list(
  CA_NB = f_calc_residuals(df_predobs_CA_NB), "CA-SA_NB" = f_calc_residuals(df_predobs_CASA_NB), 
  CA_Poisson = f_calc_residuals(df_predobs_CA_Poisson), "CA-SA_Poisson" = f_calc_residuals(df_predobs_CASA_Poisson),
  IA_NB = f_calc_residuals(df_predobs_IA_NB), "IA-SA_NB" = f_calc_residuals(df_predobs_IASA_NB), 
  IA_Poisson = f_calc_residuals(df_predobs_IA_Poisson), "IA-SA_Poisson" = f_calc_residuals(df_predobs_IASA_Poisson)
)




p_CA <- ggplot()+
  geom_dotplot(
    data = dfs$CA_NB,
    aes(
      x = as.factor(paste0(Ratio, Line)),
      y = Residuals
    ),
    color = Nord_frost[4],
    fill = Nord_frost[4],
    binaxis='y', 
    stackdir='center', 
    alpha = 0.5,
    dotsize=0.5
  )+
  geom_dotplot(
    data = dfs$CA_Poisson,
    aes(
      x = as.factor(paste0(Ratio, Line)),
      y = Residuals
    ),
    color = Nord_aurora[4],
    fill = Nord_aurora[4],
    binaxis='y', 
    stackdir='center', 
    alpha = 0.5,
    dotsize=0.5
  )+
  geom_abline(
    slope=0,
    intercept = 0
  )+
  labs(title = "CA model", x = "Conditions")+
  scale_color_manual(name = "Line", values = c(Nord_aurora, rev(Nord_frost)))+
  scale_fill_manual(name = "Line", values = c(Nord_aurora, rev(Nord_frost)))+
  theme_minimal()

p_CA_SA <- ggplot()+
  geom_dotplot(
    data = dfs$`CA-SA_NB`,
    aes(
      x = as.factor(paste0(Ratio, Line)),
      y = Residuals
    ),
    color = Nord_frost[4],
    fill = Nord_frost[4],
    binaxis='y', 
    stackdir='center', 
    alpha = 0.5,
    dotsize=0.5
  )+
  geom_dotplot(
    data = dfs$`CA-SA_Poisson`,
    aes(
      x = as.factor(paste0(Ratio, Line)),
      y = Residuals
    ),
    color = Nord_aurora[4],
    fill = Nord_aurora[4],
    binaxis='y', 
    stackdir='center', 
    alpha = 0.5,
    dotsize=0.5
  )+
  geom_abline(
    slope=0,
    intercept = 0
  )+
  labs(title = "CA-SA model", x = "Conditions")+
  scale_color_manual(name = "Line", values = c(Nord_aurora, rev(Nord_frost)))+
  scale_fill_manual(name = "Line", values = c(Nord_aurora, rev(Nord_frost)))+
  theme_minimal()

p_IA <- ggplot()+
  geom_dotplot(
    data = dfs$IA_NB,
    aes(
      x = as.factor(paste0(Ratio, Line)),
      y = Residuals
    ),
    color = Nord_frost[4],
    fill = Nord_frost[4],
    binaxis='y', 
    stackdir='center', 
    alpha = 0.5,
    dotsize=0.5
  )+
  geom_dotplot(
    data = dfs$IA_Poisson,
    aes(
      x = as.factor(paste0(Ratio, Line)),
      y = Residuals
    ),
    color = Nord_aurora[4],
    fill = Nord_aurora[4],
    binaxis='y', 
    stackdir='center', 
    alpha = 0.5,
    dotsize=0.5
  )+
  geom_abline(
    slope=0,
    intercept = 0
  )+
  labs(title = "IA model", x = "Conditions")+
  scale_color_manual(name = "Line", values = c(Nord_aurora, rev(Nord_frost)))+
  scale_fill_manual(name = "Line", values = c(Nord_aurora, rev(Nord_frost)))+
  theme_minimal()

p_IA_SA <- ggplot()+
  geom_dotplot(
    data = dfs$`IA-SA_NB`,
    aes(
      x = as.factor(paste0(Ratio, Line)),
      y = Residuals
    ),
    color = Nord_frost[4],
    fill = Nord_frost[4],
    binaxis='y', 
    stackdir='center', 
    alpha = 0.5,
    dotsize=0.5
  )+
  geom_dotplot(
    data = dfs$`IA-SA_Poisson`,
    aes(
      x = as.factor(paste0(Ratio, Line)),
      y = Residuals
    ),
    color = Nord_aurora[4],
    fill = Nord_aurora[4],
    binaxis='y', 
    stackdir='center', 
    alpha = 0.5,
    dotsize=0.5
  )+
  geom_abline(
    slope=0,
    intercept = 0
  )+
  labs(title = "IA-SA model", x = "Conditions")+
  scale_color_manual(name = "Line", values = c(Nord_aurora, rev(Nord_frost)))+
  scale_fill_manual(name = "Line", values = c(Nord_aurora, rev(Nord_frost)))+
  theme_minimal()


p <- p_CA + p_CA_SA + p_IA + p_IA_SA + plot_layout(ncol=2)
p


# Dispersion des drc substances seules ----



df_mix_coc_Cleo <- df_mix_coc_Cleo |> 
  mutate(
    Condition = paste0(Ratio, Line),
    Condition_f = as.factor(Condition)
    )

m.Poisson <- glm(Response ~ Condition_f, family = poisson, data = df_mix_coc_Cleo)
dispersion <- sum(residuals(m.Poisson, type = "pearson")^2) / m.Poisson$df.residual
dispersion
library(AER)
dispersiontest(m.Poisson)


#m.QPoisson <- survey::svyglm(Response ~ Condition_f, family = quasipoisson, data = df_mix_coc)
m.NB <- glm.nb(Response ~ Condition_f, data = df_mix_coc_Cleo)

# 1. Calculer moyenne et variance par groupe
library(dplyr)

variance_mean <- df_mix_coc_Cleo %>%
  group_by(Condition_f) %>%
  summarise(
    mean = mean(Response),
    variance = var(Response),
    n = n()
  )

theta_nb <- summary(m.NB)$theta

# 3. Créer les données pour les courbes théoriques
mean_range <- seq(0, max(variance_mean$mean) * 1.1, length.out = 100)

theoretical <- data.frame(
  mean = rep(mean_range, 3),
  variance = c(
    mean_range,  # Poisson
    dispersion * mean_range,  # Quasi-Poisson
    mean_range + mean_range^2 / theta_nb  # Negative Binomial
  ),
  model = rep(c("Poisson", "Quasi-Poisson", "Binomiale Negative"), 
              each = length(mean_range))
)

# 4. Créer le graphique
ggplot() +
  # Lignes théoriques
  geom_line(data = theoretical, 
            aes(x = mean, y = variance, color = model, linetype = model), 
            linewidth = 1.2) +
  # Points observés
  geom_point(data = variance_mean, 
             aes(x = mean, y = variance), 
             size = 4, color = "black", alpha = 0.5) +
  # Labels des conditions
  geom_text(data = variance_mean, 
            aes(x = mean, y = variance, label = Condition_f), 
            vjust = -1.2, hjust = 0.5, size = 3.5, fontface = "bold") +
  # Mise en forme
  labs(x = "Moyenne (μ)", 
       y = "Variance (σ²)", 
       title = "Relation Variance-Moyenne : Choix du modèle de distribution",
       subtitle = "Les points noirs représentent les valeurs observées par condition",
       color = "Distribution théorique",
       linetype = "Distribution théorique") +
  theme_minimal(base_size = 12) +
  theme(
    legend.position = "bottom",
    legend.title = element_text(face = "bold"),
    plot.title = element_text(face = "bold", size = 14),
    plot.subtitle = element_text(size = 10, color = "gray30")
  ) +
  scale_color_manual(values = c(
    "Poisson" = "gray50", 
    "Quasi-Poisson" = "#E41A1C", 
    "Binomiale Negative" = "#377EB8"
  )) +
  scale_linetype_manual(values = c(
    "Poisson" = "dashed",
    "Quasi-Poisson" = "solid",
    "Binomiale Negative" = "solid"
  )) +
  guides(color = guide_legend(nrow = 3), 
         linetype = guide_legend(nrow = 3))

# Afficher aussi le tableau des valeurs
print(variance_mean)
