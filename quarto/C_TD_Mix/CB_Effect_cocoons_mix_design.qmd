...
To be able to study the mixture and based on the Jonker simple interaction model, we need to see if we can consider that the dose-response curves of each molecule have common maximums and common slopes. Therefore, we need to compare different dose-response models where certain parameters are fitted indepently or not.
...
```{r, include=F}
ggsave(
  filename="Repro_comp_drc_models.png", 
  plot=plot_save, 
  width=12, height = 8, 
  path=here::here("fig/Single_expo")
  )
ggsave(
  filename="Repro_comp_drc_models.svg", 
  plot=plot_save, 
  width=12, height = 8, 
  path=here::here("fig/Single_expo")
  )
...
```{r, include=F}
ggsave(
  filename="Repro_drc_mixmodel.png", 
...
ggsave(
  filename="Repro_drc_mixmodel.svg", 
  plot=plot_drc.4.show, 
  width=5, 
  height = 4, 
  path=here::here("fig/Single_expo")
...
bartlett.test(residuals(drc.4), as.factor(subset(df_repro_alive, t==28)$Dose))
```
...
```{r, include=F}
ggsave(
  filename="Repro_design_effect_range.png", 
  plot=plot_drc_th, 
  width=8, 
  height = 4, 
  path=here::here("fig/Mixture")
...
ggsave(
  filename="Repro_design_effect_range.svg", 
  plot=plot_drc_th, 
  width=8, 
  height = 4, 
  path=here::here("fig/Mixture")
